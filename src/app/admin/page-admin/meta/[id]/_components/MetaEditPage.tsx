...
import { _pageMetaUpdateAction } from '../../_data/actions/PageMetaActions';




const title = "Edit Meta"
...
export default function MetaEditModal({ id }: PropInterface) {
    const { 
        data, 
        errors,
        toggleModal,
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault(); // Move this to the top to prevent page refresh immediately
        clearErrors();
        // 1. Validate form
        const validation = validateForm(); 
        // Note: If validateForm is async, use: const validation = await validateForm();
        if (!validation.isValid) {
            // Fallback message if specific fields aren't found
            const firstError = validation.errors.title || 
                            validation.errors.description ||
                            validation.errors.slug
            toast.warn(firstError);
            return; // Function stops here if invalid
        }
        ...
        try {
            const res = await _pageMetaUpdateAction(id, formData);
...
function KeywordInput(){
    ...
    return (
        <div className='w-full'>
            ...
            {data.keywords.length > 0 && data.keywords.map((i, key) => (
                <span 
                    key={key} 
